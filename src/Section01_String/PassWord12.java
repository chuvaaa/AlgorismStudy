 ...
 * @description SECTION01 12 문자열 압축하기.
...
public class PassWord12 {
    ...
    public static String solution(String str, int cnt) {
        ...
        for (int i=0; i<cnt; i++){
            answer += getDecodeStr(str.substring(i*fixLen, (i+1)*fixLen));
        }

        return answer;
    }

    protected static String getDecodeStr(String str){
        String binaryStr = str.replaceAll("#", "1").replaceAll("\\*", "0");
        int binaryNum = Integer.parseInt(binaryStr, 2);
        return String.valueOf((char)binaryNum);
    }
